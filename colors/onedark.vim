...
call s:h("VertSplit", { "fg": s:yellow }) " the column separating vertically split windows
...
call s:h("QuickFixLine", { "fg": s:black, "bg": s:white, "cterm": "bold" }) " Current quickfix item in the quickfix window.
...
call s:h("StatusLine", { "fg": s:black, "bg": s:yellow, "cterm": "bold" }) " status line of current window
call s:h("StatusLineNC", { "fg": s:black, "bg": s:dark_yellow }) " status lines of not-current windows Note: if this is equal to "StatusLine" Vim will use "^^^" in the status line of the current window.
